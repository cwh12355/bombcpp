...
{
public:
static T all_sum(std::vector<T> const &attr)
{
    T res = {};
...
// }
//打印auto推导；
std::vector<T> const  pri_auto(int n,std::vector<T> &attr){
...
static void pri_auto(std::vector<T>  & attr){
    tmp<T> temp;
...
int main()
{
    ...
    std::vector<T> out = temp_i.pri_auto(4,sum_data_i);
    cout << out << endl;
